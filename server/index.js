...
app.patch("/update-supplier/:supplier_id", verifyToken, async (req, res) => {
    ...
    try {
        await db.query(`UPDATE supplier SET ${setQuery} WHERE supplier_id = $${keys.length + 1}`, [...values, reqId]);
...
app.patch("/update-order-discount/:discount_id", verifyToken, async (req, res) => {
    let reqId = parseInt(req.params.discount_id);
    let body = req.body;
    let fields = [
        "discount_name",
        ...
        jwt.verify(
            ...
            async (err, account) => {
                ...
                else
                {
                    ...
                    await db.query(`UPDATE discount SET ${setQuery} WHERE discount_id = $${keys.length + 2}`, [...values, employeeId, reqId]);
