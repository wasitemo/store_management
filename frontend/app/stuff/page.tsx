...
export default function StuffPage() {
  ...
  const [meta, setMeta] = useState({ stuff_category: [], stuff_brand: [], supplier: [] });
  ...
  const loadAll = async () => {
    ...
    setMeta(metaJson.data || { stuff_category: [], stuff_brand: [], supplier: [] });
  };
  ...
  const save = async () => {
    ...
    if (payload.has_sn !== undefined) payload.has_sn = payload.has_sn ? 'true' : 'false';
    if (payload.current_sell_price !== undefined) payload.current_sell_price = String(payload.current_sell_price);
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center">
          <div className="bg-white p-6 w-[500px] space-y-4 rounded-xl">
            ...
            {!editId && (
              <>
                <input placeholder="Code" className="input" value={form.stuff_code} onChange={e => setForm({ ...form, stuff_code: e.target.value })}/>
                <input placeholder="Name" className="input" value={form.stuff_name} onChange={e => setForm({ ...form, stuff_name: e.target.value })}/>
                <input placeholder="Variant" className="input" value={form.stuff_variant} onChange={e => setForm({ ...form, stuff_variant: e.target.value })}/>
                <input placeholder="SKU" className="input" value={form.stuff_sku} onChange={e => setForm({ ...form, stuff_sku: e.target.value })}/>
                <input placeholder="Price" className="input" value={form.current_sell_price} onChange={e => setForm({ ...form, current_sell_price: e.target.value })}/>

                <select className="input" value={form.stuff_category_id} onChange={e => setForm({ ...form, stuff_category_id: e.target.value })}>
                  <option value="">Category</option>
                  {meta.stuff_category.map((c:any)=> <option key={c.stuff_category_id} value={c.stuff_category_id}>{c.stuff_category_name}</option>)}
                </select>

                <select className="input" value={form.stuff_brand_id} onChange={e => setForm({ ...form, stuff_brand_id: e.target.value })}>
                ...
                <input placeholder="Barcode" className="input" value={form.barcode} onChange={e => setForm({ ...form, barcode: e.target.value })}/>
                <label className="flex gap-2">
                  <input type="checkbox" checked={form.has_sn} onChange={e => setForm({ ...form, has_sn: e.target.checked })}/>
                  Has Serial Number
                </label>
              </>
            )}
